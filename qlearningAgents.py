...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        sampleUpdate = reward + self.discount * self.getValue(nextState)
        newQvalue = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * sampleUpdate
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in features:
            qValue = qValue + features[feature] * weights[feature]
        return qValue
