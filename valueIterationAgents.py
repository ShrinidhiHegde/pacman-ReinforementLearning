            ...
            for state in self.mdp.getStates():
                ...
                if not self.mdp.isTerminal(state):
                    for action in self.mdp.getPossibleActions(state):
                        curValue.append(self.computeQValueFromValues(state, action))
                    values[state] = max(curValue)
            self.values = values
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(curState):
                for action in self.mdp.getPossibleActions(curState):
                    curValue.append(self.computeQValueFromValues(curState, action))
                values[curState] = max(curValue)
                self.values = values
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            ...
            if not self.mdp.isTerminal(state):
                for action in self.mdp.getPossibleActions(state):
                    curValue.append(self.computeQValueFromValues(state, action))
                diff = abs(self.values[state] - max(curValue))
        ...
        for i in range(self.iterations):
            if queue.isEmpty():
                ...
                if not self.mdp.isTerminal(state):
                    curValue1 = []
                    for action in self.mdp.getPossibleActions(state):
                        curValue1.append(self.computeQValueFromValues(state, action))
                    self.values[state] = max(curValue1)
                for pred in predecessors[state]:
                    curValue1 = []
                    if not self.mdp.isTerminal(pred):
                        for action in self.mdp.getPossibleActions(pred):
                            curValue1.append(self.computeQValueFromValues(pred, action))
                        diff = abs(self.values[pred] - max(curValue1))
                        if diff > self.theta:
